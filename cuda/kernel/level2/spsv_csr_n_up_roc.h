...
spsv_csr_n_up_roc_analysis_kernel(
    const T m,
    ...
    T first_row = m - 1 - (row_offset + blockIdx.x * (blockDim.x / WARP_SIZE));  // backward
    T row = first_row - wid;
    ...
    T local_max = 0;
    // j移动范围[row_end, row_begin)(递减)
    T row_begin = csr_row_ptr[row] - 1;     // *
    T row_end = csr_row_ptr[row + 1] - 1;   // *
    T j = row_end - lid;                    // *
    T local_col = __ldg(&csr_col_idx[j]);
    while (j > row_begin && local_col > first_row) {    // *
        T local_done = done_array[local_col];
        local_max = max(local_done, local_max);
        j -= (local_done != 0) * WARP_SIZE;         // *
        if (local_done != 0 && j > row_begin) {     // *
            local_col = __ldg(&csr_col_idx[j]);
        }
    }
    while (j > row_begin && local_col > row) {      // *
        T local_idx = first_row - local_col;        // *
...
spsv_csr_n_up_roc_analysis(
    ...
    for (T row_offset = 0, row_remain = m; row_offset < m; row_offset += rowPerGrid, row_remain -= rowPerGrid) {
        ...
        spsv_csr_n_up_roc_analysis_kernel<BLOCKSIZE, WARP_SIZE><<<blockPerGrid, threadPerBlock, 0, handle->stream>>>(
            m,
            csr_row_ptr,
            csr_col_idx,
            done_array,
...
__global__ static void
spsv_csr_n_up_roc_solve_kernel(
    const T* __restrict__ csr_row_ptr,
    const T* __restrict__ csr_col_idx,
    ...
    T lid = threadIdx.x & (WARP_SIZE - 1);
    T wid = threadIdx.x / WARP_SIZE;
    T idx = row_offset + blockIdx.x * (blockDim.x / WARP_SIZE) + wid; // backward
    ...
    // j移动范围[row_end, row_begin)(递减)
    T row_begin = __ldg(&csr_row_ptr[row]) - 1;
    T row_end = __ldg(&csr_row_ptr[row + 1]) - 1;
    double local_sum = {};
    if (lid == 0) {
...
spsv_csr_n_up_roc_solve(
    ...
    for (T row_offset = 0, row_remain = m; row_offset < m; row_offset += rowPerGrid, row_remain -= rowPerGrid) {
        blockPerGrid = dim3((min(row_remain, rowPerGrid) - 1) / (BLOCKSIZE / WARP_SIZE) + 1);
        spsv_csr_n_up_roc_solve_kernel<BLOCKSIZE, WARP_SIZE><<<blockPerGrid, threadPerBlock, 0, handle->stream>>>(
            csr_row_ptr,
            csr_col_idx,
            csr_val,
            m,
