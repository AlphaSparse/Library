...
template<int block_size,
         ...
         bool ORDER_ROW>
static __global__ void
csrspmm_row_split_main_kernel_v2(
  T M, T N, T K, T nnz, W alpha, 
  const T* __restrict__ csr_row_ptr,
  const T* __restrict__ csr_col_ind,
  const U* __restrict__ csr_val,
  ...
  T ldc)
{
  ...
  for (T i = rowstart; i < rowend; i += warp_size) {
    ...
    for (T j = 0; j < warp_size; j += UNROLL_LEN) {
      for (T jj = 0; jj < UNROLL_LEN; ++jj) {
        T rowid_B = s_col_ind[local_offset + j + jj];
        U val = s_val[local_offset + j + jj];
        #pragma unroll
        for (int k = 0; k < factor; k++) {
          T colid = block_colid + k * warp_size + warp_laneid; 
          if (ORDER_ROW) {
            sum[jj][k] += val * matB[rowid_B * ldb + colid];
          }
          else {
            sum[jj][k] += val * matB[colid * ldb + rowid_B];
          }
        }
      }
      eleid += UNROLL_LEN;
  ...
  for (T i = UNROLL_LEN / 2; i > 0; i = i >> 1) {
    for (T j = 0; j < i; ++j) {
      #pragma unroll
      for (int k = 0; k < factor; ++k) {
        sum[j][k] += sum[j+i][k];
  ...
  for (int k = 0; k < factor; k++) {
    T colid = block_colid + k * warp_size + warp_laneid; 
    if (ORDER_ROW) {
      matC[rowid * ldc + colid] = alpha * sum[0][k] + beta * matC[rowid * ldc + colid];
    }
    else {
      matC[colid * ldc + rowid] = alpha * sum[0][k] + beta * matC[colid * ldc + rowid];
    }
  }
}
                ...
                void* externalBuffer)
{
  ...
  int remainder = N % (warp_size * factor);
  const int block_size = RefThreadPerBlock;
  T Mdim_worker = M;
  T Ndim_worker = N - remainder;
  T Ndim_threadblock = CEIL(Ndim_worker, warp_size * factor);
  T Ndim_thread_per_tb = warp_size;
  T Mdim_thread_per_tb = CEIL(block_size, Ndim_thread_per_tb);
  T Mdim_threadblock = CEIL(Mdim_worker, Mdim_thread_per_tb);
  ...
  dim3 blockDim(Ndim_thread_per_tb, Mdim_thread_per_tb, 1);
  if (warp_size == 32 && factor == 4) {
    csrspmm_row_split_main_kernel_v2<block_size, 32, T, U, V, W, 4, ORDER_ROW>
      <<<gridDim, blockDim, 0, handle->stream>>>(
        M, N, K, nnz, alpha,
        csr_row_ptr, csr_col_ind, csr_val, matB, ldb,
        beta, matC, ldc);
  ...
  if (warp_size == 32 && factor == 2) {
    csrspmm_row_split_main_kernel_v2<block_size, 32, T, U, V, W, 2, ORDER_ROW>
      <<<gridDim, blockDim, 0, handle->stream>>>(
        M, N, K, nnz, alpha,
        csr_row_ptr, csr_col_ind, csr_val, matB, ldb,
        beta, matC, ldc);
  ...
  if (warp_size == 32 && factor == 1) {
    csrspmm_row_split_main_kernel_v2<block_size, 32, T, U, V, W, 1, ORDER_ROW>
      <<<gridDim, blockDim, 0, handle->stream>>>(
        M, N, K, nnz, alpha,
        csr_row_ptr, csr_col_ind, csr_val, matB, ldb,
        beta, matC, ldc);
  ...
  if (warp_size == 16 && factor == 1) {
    csrspmm_row_split_main_kernel_v2<block_size, 16, T, U, V, W, 1, ORDER_ROW>
      <<<gridDim, blockDim, 0, handle->stream>>>(
        M, N, K, nnz, alpha,
        csr_row_ptr, csr_col_ind, csr_val, matB, ldb,
        beta, matC, ldc);
  ...
  if (warp_size == 8 && factor == 1) {
    csrspmm_row_split_main_kernel_v2<block_size, 8, T, U, V, W, 1, ORDER_ROW>
      <<<gridDim, blockDim, 0, handle->stream>>>(
        M, N, K, nnz, alpha,
        csr_row_ptr, csr_col_ind, csr_val, matB, ldb,
        beta, matC, ldc);
  ...
  if (warp_size == 4 && factor == 1) {
    csrspmm_row_split_main_kernel_v2<block_size, 4, T, U, V, W, 1, ORDER_ROW>
      <<<gridDim, blockDim, 0, handle->stream>>>(
        M, N, K, nnz, alpha,
        csr_row_ptr, csr_col_ind, csr_val, matB, ldb,
        beta, matC, ldc);
