...
#define C_IMPL(ONAME, TYPE)                                                                   \
  ...
  {                                                                                           \
        return create_csr5(A, indexing, rows, cols, rows_start, rows_end, col_indx, values);   \
  }
  
C_IMPL(alphasparse_s_create_csr5, float);
C_IMPL(alphasparse_d_create_csr5, double);
#undef C_IMPL