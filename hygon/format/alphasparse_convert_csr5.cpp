...
#include "convert_csr5_coo.hpp"

alphasparseStatus_t convert_csr5_datatype_coo(const internal_spmat source,
                                                    internal_spmat *dest,
                                                    alphasparse_datatype_t datatype) {
  if (datatype == ALPHA_SPARSE_DATATYPE_FLOAT) {
    return convert_csr5_coo<ALPHA_INT, float, _internal_spmat>(source, dest);
  } else if (datatype == ALPHA_SPARSE_DATATYPE_DOUBLE) {
    return convert_csr5_coo<ALPHA_INT, double, _internal_spmat>(source, dest);
  } else {
...
alphasparseStatus_t convert_sell_datatype_format(const internal_spmat source,
  ...
  if (format == ALPHA_SPARSE_FORMAT_COO) {
    return convert_csr5_datatype_coo(source, dest, datatype);
  }
  else {
    return ALPHA_SPARSE_STATUS_INVALID_VALUE;
  }
}
alphasparseStatus_t alphasparse_convert_csr5(
...
alphasparseStatus_t alphasparse_convert_csr5_internal(
    const alphasparse_matrix_t source,       
    const bool SHORT_BINNING,
    const alphasparseOperation_t operation, /* as is, transposed or conjugate transposed */
    alphasparse_matrix_t *dest) {
    return alphasparse_convert_csr5(source,SHORT_BINNING, operation, dest);
}