...
alphasparseStatus_t trsv_csc_n_hi_plain(const TYPE alpha, const internal_spmat A, const TYPE *x, TYPE *y)
{
    TYPE diag[A->cols]; 
...
template <typename TYPE>
alphasparseStatus_t trsv_csc_n_lo_plain(const TYPE alpha, const internal_spmat A, const TYPE *x, TYPE *y)
{
    TYPE diag[A->cols]; 
    memset(diag, '\0', A->cols * sizeof(TYPE));
...
template <typename TYPE>
alphasparseStatus_t trsv_csc_u_hi_plain(const TYPE alpha, const internal_spmat A, const TYPE *x, TYPE *y)
{
    //TYPE alphax[A->cols]; //???��alph*x???????????????ac??y???????????????????????��
    //memset(alphax, '\0', A->cols * sizeof(TYPE));
    for (ALPHA_INT c = 0; c < A->cols; c++)
...
template <typename TYPE>
alphasparseStatus_t trsv_csc_u_lo_plain(const TYPE alpha, const internal_spmat A, const TYPE *x, TYPE *y)
{
    //TYPE alphax[A->cols]; //???��alph*x???????????????ac??y???????????????????????��
    //memset(alphax, '\0', A->cols * sizeof(TYPE));
    for (ALPHA_INT c = 0; c < A->cols; c++)
