...
alphasparseStatus_t trsv_sky_n_hi_conj_plain(const TYPE alpha, const internal_spmat A, const TYPE *x, TYPE *y)
{
    TYPE diag[A->cols];
...
template <typename TYPE>
alphasparseStatus_t trsv_sky_n_lo_conj_plain(const TYPE alpha, const internal_spmat A, const TYPE *x, TYPE *y)
{
    TYPE diag[A->cols];
    memset(diag, '\0', A->rows * sizeof(TYPE));
    for (ALPHA_INT r = 1; r < A->rows + 1; r++)
...
template <typename TYPE>
alphasparseStatus_t trsv_sky_u_hi_conj_plain(const TYPE alpha, const internal_spmat A, const TYPE *x, TYPE *y)
{
    for (ALPHA_INT r = 0; r < A->rows; r++)
    {
        TYPE temp = TYPE{};
...
template <typename TYPE>
alphasparseStatus_t trsv_sky_u_lo_conj_plain(const TYPE alpha, const internal_spmat A, const TYPE *x, TYPE *y)
{
    for (ALPHA_INT c = A->cols - 1; c >= 0; c--)
    {
        TYPE temp = TYPE{};
