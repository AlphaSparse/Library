...
template <typename J>
alphasparseStatus_t spmm_csr_trans_plain(const internal_spmat A, const internal_spmat B, internal_spmat *matC)
{
    internal_spmat transposed_mat;
    transpose_csr<J>(A, &transposed_mat);
    alphasparseStatus_t status = spmm_csr_plain<J>(transposed_mat, B, matC);
