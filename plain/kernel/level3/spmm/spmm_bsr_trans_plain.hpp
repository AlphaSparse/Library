...
alphasparseStatus_t spmm_bsr_trans_plain(const internal_spmat A, const internal_spmat B, internal_spmat *matC)
{
    internal_spmat transposed_mat;
    transpose_bsr<TYPE>(A, &transposed_mat); //��matת��
    alphasparseStatus_t status = spmm_bsr_plain<TYPE>(transposed_mat, B, matC); //�ٵ��ó˷�
