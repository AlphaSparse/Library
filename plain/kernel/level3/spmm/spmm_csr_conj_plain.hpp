...
alphasparseStatus_t spmm_csr_conj_plain(const internal_spmat A, const internal_spmat B, internal_spmat *matC)
{
    internal_spmat conjugated_mat;
    transpose_conj_csr<TYPE>(A, &conjugated_mat);
    alphasparseStatus_t status = spmm_csr_plain<TYPE>(conjugated_mat, B, matC);
