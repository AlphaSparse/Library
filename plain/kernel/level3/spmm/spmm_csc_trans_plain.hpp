...
alphasparseStatus_t spmm_csc_trans_plain(const internal_spmat A, const internal_spmat B, internal_spmat *matC)
{
    internal_spmat transposed_mat;
    transpose_csc<TYPE>(A, &transposed_mat);
    alphasparseStatus_t status = spmm_csc_plain<TYPE>(transposed_mat, B, matC);
