...
alphasparseStatus_t spmm_bsr_conj_plain(const internal_spmat A, const internal_spmat B, internal_spmat *matC)
{
    internal_spmat conjugated_mat;
    transpose_conj_bsr<TYPE>(A, &conjugated_mat); //��matת��
    alphasparseStatus_t status = spmm_bsr_plain<TYPE>(conjugated_mat, B, matC); //�ٵ��ó˷�
