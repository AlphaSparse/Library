...
// alphasparseStatus_t gemm_csc_row_plain(const TYPE alpha, const internal_spmat mat, const TYPE *x, const ALPHA_INT columns, const ALPHA_INT ldx, const TYPE beta, TYPE *y, const ALPHA_INT ldy)
// {
...
template <typename TYPE>
alphasparseStatus_t
gemm_csc_row_plain(const TYPE alpha, const internal_spmat mat, const TYPE *x, const ALPHA_INT columns, const ALPHA_INT ldx, const TYPE beta, TYPE *y, const ALPHA_INT ldy)
{
    ...
    trans->rows = mat->cols;
    trans->cols = mat->rows;
    gemm_csr_row_trans_plain(alpha, trans, x, columns, ldx, beta, y, ldy);
    return ALPHA_SPARSE_STATUS_SUCCESS;
}
