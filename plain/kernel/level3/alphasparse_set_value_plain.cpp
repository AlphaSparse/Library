...
#include "set/set_value_csr_plain.hpp"
#include "set/set_value_coo_plain.hpp"
#include "set/set_value_bsr_plain.hpp"
#include "set/set_value_csc_plain.hpp"

template <typename J>
alphasparseStatus_t alphasparse_set_value_template_plain(alphasparse_matrix_t A, 
                        const ALPHA_INT row, 
                        const ALPHA_INT col,
                        const J value)
{
...
#define C_IMPL(ONAME, TYPE)                                             \
    alphasparseStatus_t ONAME (alphasparse_matrix_t A,  \
                                    ...
                                    const TYPE value)                   \
    {                                                                   \
        return alphasparse_set_value_template_plain(A, row, col, value);      \
    }   
C_IMPL(alphasparse_s_set_value_plain, float);
C_IMPL(alphasparse_d_set_value_plain, double);
C_IMPL(alphasparse_c_set_value_plain, ALPHA_Complex8);
C_IMPL(alphasparse_z_set_value_plain, ALPHA_Complex16);
#undef C_IMPL