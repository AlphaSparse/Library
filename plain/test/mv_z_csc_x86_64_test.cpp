...
static void mkl_mv(const int argc, const char *argv[], const char *file,
                   ...
                   const MKL_Complex16 beta, MKL_Complex16 **ret_y,
                   size_t *ret_size_y) {
  MKL_INT m, k, nnz;
...
// thread_num, const ALPHA_Complex16 alpha, const ALPHA_Complex16 beta, ALPHA_Complex16
...
static void alpha_mv(const int argc, const char *argv[], const char *file,
                   ...
                   const ALPHA_Complex16 beta, ALPHA_Complex16 **ret_y,
                   size_t *ret_size_y, ALPHA_Complex16 **ret_x,
                   size_t *ret_size_x) {
  ALPHA_INT m, k, nnz;
  ...
  alpha_call_exit(alphasparse_z_mv_plain(transA, alpha, cscA, descr, x, beta, y),
                "alphasparse_z_mv");
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    mkl_mv(argc, argv, file, thread_num, mkl_alpha, mkl_beta, &mkl_y,
           &size_mkl_y);
    // status = check_d((ALPHA_Complex16 *)mkl_y, size_mkl_y * 2, (ALPHA_Complex16
    // *)alpha_y, size_alpha_y * 2); status = check_z_l2((ALPHA_Complex16 *)mkl_y,
