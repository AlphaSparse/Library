...
static void alpha_trsm(const int argc, const char *argv[], const char *file,
  ...
  alpha_timer_t timer;
  alpha_timing_start(&timer);
  alpha_call_exit(alphasparse_s_trsm_plain(transA, alpha, csrA, descr, layout, x,
                                        columns, ldx, y, ldy),
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    //        size_alpha_y * 2);
    status = check_s_l3((float *)mkl_y, ldc, size_mkl_y, alpha_y, ldc, size_alpha_y,
                        NULL, alpha_x, ldx, alpha_y, ldc, alpha_alpha, alpha_alpha,
                        argc, argv);
    alpha_free(mkl_y);
  }
  printf("\n");
