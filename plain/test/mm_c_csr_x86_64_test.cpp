...
static void mkl_mm(const int argc, const char *argv[], const char *file,
  ...
  mkl_call_exit(mkl_sparse_c_create_coo(&coo, SPARSE_INDEX_BASE_ZERO, m, k, nnz,
  ...
  mkl_call_exit(mkl_sparse_c_mm(transA, alpha, csr, descr, layout, x, columns,
                                ldx, beta, y, ldy),
                "mkl_sparse_c_mm");
...
static void alpha_mm(const int argc, const char *argv[], const char *file,
  ...
  alpha_call_exit(alphasparse_c_mm_plain(transA, alpha, csr, descr, layout, x,
...
int main(int argc, const char *argv[]) {
  ...
  alpha_mm(argc, argv, file, thread_num, alpha, beta, &alpha_x, &size_alpha_x, &ldx,
         &alpha_y, &size_alpha_y, &ldy);
  ...
  if (check) {
    ...
    mkl_mm(argc, argv, file, thread_num, mkl_alpha, mkl_beta, &mkl_y,
           &size_mkl_y);
    printf(",");
    //        check_s(float *)mkl_y, size_mkl_y * 2, (float *)alpha_y, size_alpha_y
    //        * 2);
    status = check_c_l3((ALPHA_Complex8 *)mkl_y, ldy, size_mkl_y, alpha_y, ldy,
                        size_alpha_y, NULL, alpha_x, ldx, alpha_y, ldy, alpha, beta,
                        argc, argv);
    alpha_free(mkl_y);
  }
  printf("\n");
