...
static void mkl_trsm(const int argc, const char *argv[], const char *file, int thread_num,
                     const MKL_Complex16 alpha, MKL_Complex16 **ret, size_t *size) {
  MKL_INT m, k, nnz;
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    mkl_call_exit(mkl_sparse_z_trsm(transA, alpha, bsrA, descr, layout, x, columns, ldx, y, ldy),
                  "mkl_sparse_z_trsm");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter, "mkl_sparse_z_trsm", total_time / iter);
  // alpha_timing_end(&timer);
...
static void mkl_trsm_bsr(const int argc, const char *argv[], const char *file, int thread_num,
  ...
  // #ifdef DEBUG
  //     printf(" mkl_sparse_z_create_coo starts \n");
  // #endif
  mkl_sparse_z_create_coo(&cooA, base, m, k, nnz, row_index, col_index, values);
  // #ifdef DEBUG
  //     printf(" mkl_sparse_z_create_coo ends \n");
  // #endif
  // #ifdef DEBUG
  ...
  // alpha_timing_start(&timer);
  // #ifdef DEBUG
  //    //     printf(" mkl_sparse_z_trsm starts \n");
  // #endif
...
static void alpha_trsm(const int argc, const char *argv[], const char *file, int thread_num,
                     const ALPHA_Complex16 alpha, ALPHA_Complex16 **ret, size_t *size) {
  ALPHA_INT m, k, nnz;
  ...
  for (int i = 0; i < iter; i++) {
    ...
    alpha_timing_start(&timer);
    alpha_call_exit(
        alphasparse_z_trsm_plain(transA, alpha, bsrA, descr, b_layout, x, columns, ldx, y, ldy),
        "alphasparse_z_trsm");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter, "alphasparse_z_trsm_plain",
         total_time / iter);
  ;
  // alpha_timing_end(&timer);
