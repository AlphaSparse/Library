...
static void mkl_mm(const int argc, const char *argv[], const char *file,
                   int thread_num, MKL_Complex8 alpha, MKL_Complex8 beta,
                   MKL_Complex8 **ret_y, size_t *ret_size_y) {
  MKL_INT m, k, nnz;
  ...
  alphasparse_c_create_coo(&cooA, base_ict, m, k, nnz,
  ...
  mkl_call_exit(mkl_sparse_c_mm(transA, alpha, cscA, descr, layout, x, columns,
                                ldx, beta, y, ldy),
                "mkl_sparse_c_mm");
...
// thread_num, ALPHA_Complex8 alpha, ALPHA_Complex8 beta, ALPHA_Complex8 **ret_y,
// size_t *ret_size_y)
static void alpha_mm(const int argc, const char *argv[], const char *file,
                   int thread_num, ALPHA_Complex8 alpha, ALPHA_Complex8 beta,
                   ALPHA_Complex8 **ret_x, size_t *ret_size_x, ALPHA_INT *ret_ldx,
                   ALPHA_Complex8 **ret_y, size_t *ret_size_y, ALPHA_INT *ret_ldy) {
  ALPHA_INT m, k, nnz;
  ...
  alpha_call_exit(alphasparse_c_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, m, k,
  ...
  alpha_call_exit(alphasparse_c_mm_plain(transA, alpha, csc, descr, layout, x,
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    // status = check_c_l3((ALPHA_Complex8 *)mkl_y, ldy, size_mkl_y, alpha_y, ldy,
    // size_alpha_y, NULL, alpha_x, ldx, alpha_y, ldy, alpha, beta, argc, argv);
    alpha_free(mkl_y);
  }
