...
static void mkl_spmmd(const int argc, const char *argv[], const char *file,
  ...
  mkl_sparse_d_spmmd(transA, csrA, csrB, layout, C, ldc);
...
static void alpha_spmmd(const int argc, const char *argv[], const char *file,
  ...
  alpha_timer_t timer;
  alpha_timing_start(&timer);
  alpha_call_exit(alphasparse_d_spmmd_plain(transA, csrA, csrB, layout, C, ldc),
                "alphasparse_d_spmmd");
  alpha_timing_end(&timer);
  ...
  if (check) {
    ...
    status =
        check_d_l3((double *)mkl_C, ldc, size_mkl_C, alpha_C, ldc, size_alpha_C,
                   NULL, NULL, 0, alpha_C, ldc, alpha, beta, argc, argv);
    alpha_free(mkl_C);
  }
