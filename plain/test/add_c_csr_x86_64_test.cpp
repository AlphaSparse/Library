...
static void mkl_add(const int argc, const char *argv[], const char *file,
                    MKL_Complex8 alpha, sparse_index_base_t *ret_index,
  ...
  mkl_call_exit(mkl_sparse_c_add(transA, csr, alpha, csrt, &result),
                "mkl_sparse_c_add");
  ...
  mkl_call_exit(mkl_sparse_c_export_csr(result, ret_index, ret_rows, ret_cols,
...
static void alpha_add(const int argc, const char *argv[], const char *file,
                    ALPHA_Complex8 alpha, alphasparseIndexBase_t *ret_index,
  ...
  alpha_call_exit(alphasparse_c_add_plain(transA, csr, alpha, csrt, &result),
                "alphasparse_c_add_plain");

  alpha_timing_end(&timer);
  printf("%lf,%lf", alpha_timing_elapsed_time(&timer),
         alpha_timing_gflops(&timer, (double)nnz * k * 2 + m * k));

  alpha_call_exit(alphasparse_c_export_csr(result, ret_index, ret_rows, ret_cols,
                                        ret_rows_start, ret_rows_end,
                                        ret_col_index, ret_values),
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    status = check_c_l3((ALPHA_Complex8 *)mkl_values, 0, mkl_nnz, alpha_values, 0,
