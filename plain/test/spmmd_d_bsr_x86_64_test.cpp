...
static void mkl_spmmd(const int argc, const char *argv[], const char *file,
                      int thread_num, double **ret, size_t *ret_size) {
  MKL_INT m, k, nnz;
  ...
  double *C = (double*)alpha_malloc(sizeof(double) * size_C);
  MKL_INT ldc;
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    mkl_call_exit(mkl_sparse_d_spmmd(transA, bsrA, bsrB, layout, C, ldc),
                  "mkl_sparse_d_spmmd");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter, "mkl_sparse_d_spmmd",
         total_time / iter);
  // alpha_timing_end(&timer);
...
static void alpha_spmmd(const int argc, const char *argv[], const char *file,
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    alpha_call_exit(alphasparse_d_spmmd_plain(transA, bsrA, bsrB, layout, C, ldc),
                  "alphasparse_d_spmmd");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter,
         "alphasparse_d_spmmd_plain", total_time / iter);
  ;
  // alpha_timing_end(&timer);
...
int main(int argc, const char *argv[]) {
  ...
  {  // block row major
    ...
    if (check) {
      ...
      // {
      // //  printf("mkl:%f, ict:%f\n", (double)(mkl_C[i].real),
      // (double)(alpha_C[i].real));
      // }
      // status = check_d((double *)mkl_C, size_mkl_C, (double *)alpha_C,
      // size_alpha_C);
      alpha_free(mkl_C);
    }
  }
