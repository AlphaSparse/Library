...
static void mkl_set(const int argc, const char *argv[], const char *file,
  ...
  MKL_INT nvals = 500 > nnz ? 500 : nnz;
  float *changed_vals = (float*)alpha_memalign(sizeof(float) * nvals, DEFAULT_ALIGNMENT);
  MKL_INT *changed_rows =
      (MKL_INT*)alpha_memalign(sizeof(MKL_INT) * nvals, DEFAULT_ALIGNMENT);
  MKL_INT *changed_cols =
      (MKL_INT*)alpha_memalign(sizeof(MKL_INT) * nvals, DEFAULT_ALIGNMENT);
  alpha_fill_random_s((float *)changed_vals, 2, nvals);
  for (ALPHA_INT i = 0; i < nvals; i++) {
    changed_rows[i] = row_index[i];
    changed_cols[i] = col_index[i];
  }

  mkl_set_num_threads(thread_num);
  sparse_operation_t transA = mkl_args_get_transA(argc, argv);

  sparse_matrix_t coo, bsrA;
  ...
  mkl_call_exit(mkl_sparse_convert_bsr(coo, 4, SPARSE_LAYOUT_ROW_MAJOR,
  ...
  mkl_call_exit(mkl_sparse_s_update_values(bsrA, nvals, changed_rows,
                                           changed_cols, changed_vals),
                "mkl_sparse_update");

  alpha_timing_end(&timer);
  alpha_timing_elaped_time_print(&timer, "mkl_sparse_update");

  sparse_layout_t block_layout;
  MKL_INT block_size;

  mkl_call_exit(
      mkl_sparse_s_export_bsr(bsrA, ret_index, &block_layout, ret_rows,
                              ret_cols, &block_size, ret_rows_start,
...
static void alpha_set(const int argc, const char *argv[], const char *file,
  ...
  alpha_call_exit(
      alphasparse_convert_bsr(coo, 4, ALPHA_SPARSE_LAYOUT_ROW_MAJOR,
                             ALPHA_SPARSE_OPERATION_NON_TRANSPOSE, &bsrA),
      "alphasparse_convert_bsr");

  ALPHA_INT nvals = 500 > nnz ? 500 : nnz;
  float *changed_vals = (float*)alpha_memalign(sizeof(float) * nvals, DEFAULT_ALIGNMENT);
  ALPHA_INT *changed_rows =
      (ALPHA_INT*)alpha_memalign(sizeof(ALPHA_INT) * nvals, DEFAULT_ALIGNMENT);
  ALPHA_INT *changed_cols =
      (ALPHA_INT*)alpha_memalign(sizeof(ALPHA_INT) * nvals, DEFAULT_ALIGNMENT);
  alpha_fill_random_s((float *)changed_vals, 2, nvals);
  for (ALPHA_INT i = 0; i < nvals; i++) {
    changed_rows[i] = row_index[i];
  ...
  alpha_call_exit(alphasparse_s_update_values_plain(bsrA, nvals, changed_rows,
                                                 changed_cols, changed_vals),
                "alphasparse_update_plain");

  alpha_timing_end(&timer);
  alpha_timing_elaped_time_print(&timer, "alphasparse_update_plain");

  alphasparse_layout_t block_layout = ALPHA_SPARSE_LAYOUT_ROW_MAJOR;
  ALPHA_INT block_size = 0;

  alpha_call_exit(
      alphasparse_s_export_bsr(bsrA, ret_index, &block_layout, ret_rows,
                              ret_cols, &block_size, ret_rows_start,
                              ret_rows_end, ret_col_index, ret_values),
