...
static void mkl_mm(const int argc, const char *argv[], const char *file,
  ...
  float *x = (float *)(float*)alpha_memalign(sizeof(float) * size_x, DEFAULT_ALIGNMENT);
  float *y = (float *)(float*)alpha_memalign(sizeof(float) * size_y, DEFAULT_ALIGNMENT);
...
static void alpha_mm(const int argc, const char *argv[], const char *file,
                   int thread_num, float alpha, float beta, float **ret_x,
                   size_t *ret_size_x, ALPHA_INT *ret_ldx, float **ret_y,
  ...
  alpha_call_exit(alphasparse_s_mm_plain(transA, alpha, csr, descr, layout, x,
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    //        * 2);
    status = check_s_l3((float *)mkl_y, ldy, size_mkl_y, alpha_y, ldy, size_alpha_y,
