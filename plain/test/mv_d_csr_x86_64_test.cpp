...
static void mkl_mv(const int argc, const char *argv[], const char *file,
                   int thread_num, const double alpha, const double beta,
                   double **ret_y, size_t *ret_size_y) {
  MKL_INT m, k, nnz;
...
static void alpha_mv(const int argc, const char *argv[], const char *file,
                   int thread_num, const double alpha, const double beta,
                   double **ret_y, size_t *ret_size_y, double **ret_x,
                   size_t *ret_size_x) {
  ALPHA_INT m, k, nnz;
  ...
  alpha_call_exit(alphasparse_d_mv_plain(transA, alpha, csrA, descr, x, beta, y),
                "alphasparse_d_mv");
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    //        size_alpha_y * 2);
    status = check_d_l2((double *)mkl_y, size_mkl_y, alpha_y, size_alpha_y, alpha_x,
                        NULL, alpha, beta, argc, argv);
    alpha_free(mkl_y);
  }
  printf("\n");
  alpha_free(alpha_y);
