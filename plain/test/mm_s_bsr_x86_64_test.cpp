...
static void mkl_mm(const int argc, const char *argv[], const char *file,
  ...
  mkl_call_exit(mkl_sparse_s_create_coo(&coo, base, m, k, nnz, row_index,
                                        col_index, values),
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    mkl_call_exit(mkl_sparse_s_mm(transA, alpha, bsr, descr, layout, x, columns,
                                  ldx, beta, y, ldy),
                  "mkl_sparse_s_mm");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter, "mkl_sparse_s_mm",
         total_time / iter);
...
static void alpha_mm(const int argc, const char *argv[], const char *file,
  ...
  alpha_call_exit(alphasparse_s_create_coo(&coo, ALPHA_BASE, m, k, nnz, row_index,
                                        col_index, values),
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    alpha_call_exit(alphasparse_s_mm_plain(transA, alpha, bsr, descr, layout, x,
                                        columns, ldx, beta, y, ldy),
                  "alphasparse_s_mm");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter, "alphasparse_s_mm_plain",
         total_time / iter);
  ;
