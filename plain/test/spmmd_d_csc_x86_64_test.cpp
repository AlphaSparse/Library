...
static sparse_status_t alpha_convert_mkl_csc_d(alphasparse_matrix_t src,
  ...
  sparse_status_t st = mkl_sparse_d_create_csc(
      ...
      mat->row_indx, (double *)mat->values);
  return st;
}
...
static void mkl_spmmd(const int argc, const char *argv[], const char *file,
  ...
  alphasparse_d_create_coo(&cooA, ALPHA_SPARSE_INDEX_BASE_ZERO, mA, kA, nnzA,
  ...
  alphasparse_d_create_coo(&cooB, ALPHA_SPARSE_INDEX_BASE_ZERO, mB, kB, nnzB,
...
static void alpha_spmmd(const int argc, const char *argv[], const char *file,
  ...
  // create cscA
  alpha_call_exit(
      alphasparse_d_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, mA, kA, nnzA,
                              row_indexA, col_indexA, valuesA),
  ...
  // create cscB
  alpha_call_exit(
      alphasparse_d_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, mB, kB, nnzB,
                              row_indexB, col_indexB, valuesB),
      "alphasparse_d_create_coo");
  alpha_call_exit(
  ...
  alpha_timer_t timer;
  alpha_timing_start(&timer);
  alpha_call_exit(alphasparse_d_spmmd_plain(transA, cscA, cscB, layout, C, ldc),
                "alphasparse_d_spmmd");
  alpha_timing_end(&timer);
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    status =
        check_d_l3((double *)mkl_C, ldc, size_mkl_C, alpha_C, ldc, size_alpha_C,
                   NULL, NULL, 0, alpha_C, ldc, 0, 0, argc, argv);
