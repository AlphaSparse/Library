...
static void mkl_trsv(const int argc, const char *argv[], const char *file,
  ...
  // mkl_sparse_z_create_coo(&cooA, SPARSE_INDEX_BASE_ZERO, m, k, nnz,
  ...
  mkl_call_exit(mkl_sparse_z_trsv(transA, alpha, cscA, descr, x, y),
                "mkl_sparse_z_trsv");
...
static void alpha_trsv(const int argc, const char *argv[], const char *file,
  ...
  alpha_call_exit(alphasparse_z_trsv_plain(transA, alpha, csrA, descr, x, y),
                "alphasparse_z_trsv_plain");
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    // status = check_z_l2((ALPHA_Complex16 *)mkl_y, size_mkl_y, alpha_y,
    // size_alpha_y, alpha_x, NULL, alpha_alpha, zero, argc, argv);
    alpha_free(mkl_y);
  }
