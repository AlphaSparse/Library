...
static void mkl_mm(const int argc, const char *argv[], const char *file,
                   int thread_num, double alpha, double beta, double **ret_y,
                   size_t *ret_size_y) {
  MKL_INT m, k, nnz;
  ...
  mkl_call_exit(mkl_sparse_d_mm(transA, alpha, csr, descr, layout, x, columns,
                                ldx, beta, y, ldy),
                "mkl_sparse_d_mm");
...
static void alpha_mm(const int argc, const char *argv[], const char *file,
                   int thread_num, double alpha, double beta, double **ret_x,
  ...
  alpha_call_exit(alphasparse_d_mm_plain(transA, alpha, csr, descr, layout, x,
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    mkl_mm(argc, argv, file, thread_num, mkl_alpha, mkl_beta, &mkl_y,
           &size_mkl_y);
    printf(",");
    //        check_d(double *)mkl_y, size_mkl_y * 2, (double *)alpha_y,
    //        size_alpha_y * 2);
    status =
        check_d_l3((double *)mkl_y, ldy, size_mkl_y, alpha_y, ldy, size_alpha_y,
                   NULL, alpha_x, ldx, alpha_y, ldy, alpha, beta, argc, argv);
    alpha_free(mkl_y);
  }
  printf("\n");
  alpha_free(alpha_y);
