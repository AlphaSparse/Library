...
static void mkl_add(const int argc, const char *argv[], const char *file,
                    MKL_Complex16 alpha, sparse_index_base_t *ret_index,
  ...
  alpha_fill_random_z((ALPHA_Complex16 *)values, 1, nnz);
  mkl_set_num_threads(thread_num);

  sparse_operation_t transA = mkl_args_get_transA(argc, argv);

  sparse_matrix_t coo, csr, csrt, result;
  mkl_call_exit(mkl_sparse_z_create_coo(&coo, SPARSE_INDEX_BASE_ZERO, m, k, nnz,
  ...
  mkl_call_exit(mkl_sparse_z_add(transA, csr, alpha, csrt, &result),
                "mkl_sparse_z_add");
  ...
  mkl_call_exit(mkl_sparse_z_export_csr(result, ret_index, ret_rows, ret_cols,
...
static void alpha_add(const int argc, const char *argv[], const char *file,
                    ALPHA_Complex16 alpha, alphasparseIndexBase_t *ret_index,
  ...
  alpha_call_exit(alphasparse_z_add_plain(transA, csr, alpha, csrt, &result),
                "alphasparse_z_add_plain");

  alpha_timing_end(&timer);
  printf("%lf,%lf", alpha_timing_elapsed_time(&timer),
         alpha_timing_gflops(&timer, (double)nnz * k * 2 + m * k));

  alpha_call_exit(alphasparse_z_export_csr(result, ret_index, ret_rows, ret_cols,
                                        ret_rows_start, ret_rows_end,
                                        ret_col_index, ret_values),
...
int main(int argc, const char *argv[]) {
  ...
  const ALPHA_Complex16 alpha_alpha = {.real = 3., .imag = 3.};
  ...
  if (check) {
    ...
    printf(",");
    //        status = check_d(double *)mkl_values, 2 * mkl_nnz, (double
    //        *)alpha_values, 2 * alpha_nnz);
    status = check_z_l3((ALPHA_Complex16 *)mkl_values, 0, mkl_nnz, alpha_values, 0,
                        alpha_nnz, alpha_col_index, NULL, 0, alpha_values, 0, alpha,
                        beta, argc, argv);
  }
  printf("\n");
  return status;
