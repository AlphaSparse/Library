...
static sparse_status_t alpha_convert_mkl_csc_c(alphasparse_matrix_t src,
  ...
  sparse_status_t st = mkl_sparse_c_create_csc(
      ...
      mat->row_indx, (MKL_Complex8 *)mat->values);
  return st;
}

static void mkl_spmmd(const int argc, const char *argv[], const char *file,
                      int thread_num, MKL_Complex8 **ret, size_t *ret_size) {
  // 没有考虑column
  ...
  MKL_Complex8 *C = (MKL_Complex8*)alpha_malloc(sizeof(MKL_Complex8) * size_C);

  sparse_matrix_t result;
  // create cscA
  alphasparse_matrix_t cooA, alpha_cscA;
  sparse_matrix_t cscA;
  alphasparse_c_create_coo(&cooA, ALPHA_SPARSE_INDEX_BASE_ZERO, mA, kA, nnzA,
  ...
  // mkl_call_exit(mkl_sparse_c_create_coo(&coo, SPARSE_INDEX_BASE_ZERO, m, k,
  // nnz, row_index, col_index, values), "mkl_sparse_c_create_coo");
  ...
  alphasparse_c_create_coo(&cooB, ALPHA_SPARSE_INDEX_BASE_ZERO, mB, kB, nnzB,
  ...
  mkl_sparse_c_spmmd(transA, cscA, cscB, layout, C, ldc);
...
// int thread_num, ALPHA_Complex8 **ret, size_t *ret_size)
static void alpha_spmmd(const int argc, const char *argv[], const char *file,
                      int thread_num, ALPHA_Complex8 **ret, size_t *ret_size,
                      ALPHA_INT *ret_ldc) {
  ...
  //     else ldc = kA;
  // }
  ALPHA_Complex8 *C = (ALPHA_Complex8*)alpha_malloc(sizeof(ALPHA_Complex8) * size_C);
  ...
  // create cscA
  alpha_call_exit(
      alphasparse_c_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, mA, kA, nnzA,
                              row_indexA, col_indexA, valuesA),
  ...
  // create cscB
  alpha_call_exit(
      alphasparse_c_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, mB, kB, nnzB,
                              row_indexB, col_indexB, valuesB),
      "alphasparse_c_create_coo");
  alpha_call_exit(
  ...
  alpha_timer_t timer;
  alpha_timing_start(&timer);
  alpha_call_exit(alphasparse_c_spmmd_plain(transA, cscA, cscB, layout, C, ldc),
                "alphasparse_c_spmmd");
  alpha_timing_end(&timer);
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    ALPHA_Complex8 zero = {0, 0};
    status = check_c_l3((ALPHA_Complex8 *)mkl_C, ldc, size_mkl_C, alpha_C, ldc,
