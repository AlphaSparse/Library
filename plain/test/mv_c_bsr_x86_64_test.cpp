...
static void mkl_mv_coo(const int argc, const char *argv[], const char *file, int thread_num,
                       const MKL_Complex8 alpha, const MKL_Complex8 beta, MKL_Complex8 **ret_y,
                       size_t *ret_size_y) {
  MKL_INT m, k, nnz;
  ...
  MKL_Complex8 *x = (MKL_Complex8*)alpha_memalign(sizeof(MKL_Complex8) * size_x, DEFAULT_ALIGNMENT);
  MKL_Complex8 *y = (MKL_Complex8*)alpha_memalign(sizeof(MKL_Complex8) * size_y, DEFAULT_ALIGNMENT);
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    mkl_call_exit(mkl_sparse_c_mv(transA, alpha, cooA, descr, x, beta, y), "mkl_sparse_c_mv");
...
static void alpha_mv(const int argc, const char *argv[], const char *file, int thread_num,
                   const ALPHA_Complex8 alpha, const ALPHA_Complex8 beta, ALPHA_Complex8 **ret_y,
                   size_t *ret_size_y) {
  ALPHA_INT m, k, nnz;
  ...
  ALPHA_Complex8 *x = (ALPHA_Complex8*)alpha_memalign(sizeof(ALPHA_Complex8) * size_x, DEFAULT_ALIGNMENT);
  ALPHA_Complex8 *y = (ALPHA_Complex8*)alpha_memalign(sizeof(ALPHA_Complex8) * size_y, DEFAULT_ALIGNMENT);
  ...
  alpha_call_exit(alphasparse_c_create_coo(&cooA, ALPHA_SPARSE_INDEX_BASE_ZERO, m, k, nnz, row_index,
                                        col_index, values),
...
int main(int argc, const char *argv[]) {
  ...
  const ALPHA_Complex8 beta = {.real = 2, .imag = 0.};

  const MKL_Complex8 mkl_alpha = {.real = 3., .imag = 3.};
  const MKL_Complex8 mkl_beta = {.real = 2, .imag = 0};

  ALPHA_Complex8 *alpha_y;
  MKL_Complex8 *mkl_y;
  MKL_Complex8 *mkl_y20;
  MKL_Complex8 *mkl_ycoo;
  ...
  if (check) {
    ...
    // printf("mkl20 vs ict\n");
    // status = check_s((float *)mkl_y20, size_mkl_y20 * 2, (float *)alpha_y,
    // size_alpha_y * 2);

    // //sanity check
    // printf("mkl vs mkl_coo\n");
    // status = check_s((float *)mkl_y, size_mkl_y * 2, (float *)mkl_ycoo,
    // size_mkl_ycoo * 2); printf("mkl20 vs mkl_coo\n"); status = check_s((float
    // *)mkl_y20, size_mkl_y20 * 2, (float *)mkl_ycoo, size_mkl_ycoo * 2);
    // // printf("mkl sanity check\n");
    // printf("mkl vs mkl20\n");
    // status = check_s((float *)mkl_y, size_mkl_y * 2, (float *)mkl_y20,
    // size_mkl_y20 * 2);
    //  for(int i = 0 ; i < size_mkl_y*2 ; i++){
    //     printf("mkl20 %lf ,ict %lf, diff %lf \n", ((float
    //     *)mkl_y20)[i],((float *)mkl_y)[i],((float *)mkl_y20)[i]-((float
    //     *)mkl_y)[i]);
    // }
    // printf("ict vs mkl_coo\n");
    // status = check_s((float *)alpha_y, size_alpha_y * 2, (float *)mkl_ycoo,
    // size_mkl_ycoo * 2);

    // for(int i = 0 ; i < size_mkl_y*2 ; i++){
    //     printf("mkl %lf ,ict %lf, diff %lf\n", ((float *)mkl_y)[i],((float
    //     *)alpha_y)[i],((float *)mkl_y)[i] - ((float *)alpha_y)[i]);
    // }

    // for(int i = 0 ; i < size_mkl_y*2 ; i++){
    //     printf("mkl20 %lf ,ict %lf, diff %lf \n", ((float
    //     *)mkl_y20)[i],((float *)alpha_y)[i],((float *)mkl_y20)[i]-((float
    //     *)alpha_y)[i]);
    // }
