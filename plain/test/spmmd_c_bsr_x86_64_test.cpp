...
static void mkl_spmmd(const int argc, const char *argv[], const char *file,
  ...
  mkl_sparse_c_create_coo(&cooB, SPARSE_INDEX_BASE_ZERO, m, k, nnz, row_index,
                          col_index, values);
  mkl_call_exit(
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    mkl_call_exit(mkl_sparse_c_spmmd(transA, bsrA, bsrB, layout, C, ldc),
                  "mkl_sparse_c_spmmd");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter, "mkl_sparse_c_spmmd",
         total_time / iter);
  // alpha_timing_end(&timer);
...
static void alpha_spmmd(const int argc, const char *argv[], const char *file,
  ...
  alphasparse_c_create_coo(&cooB, ALPHA_SPARSE_INDEX_BASE_ZERO, m, k, nnz,
                          row_index, col_index, values);
  alpha_call_exit(
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    alpha_call_exit(alphasparse_c_spmmd_plain(transA, bsrA, bsrB, layout, C, ldc),
                  "alphasparse_c_spmmd");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter,
         "alphasparse_c_spmmd_plain", total_time / iter);
  ;
  // alpha_timing_end(&timer);
...
int main(int argc, const char *argv[]) {
  ...
  {  // block row major
    ...
    if (check) {
      ...
      // {
      // //  printf("mkl:%f, ict:%f\n", (float)(mkl_C[i].real),
      // (float)(alpha_C[i].real));
      // }
      // status = check_s((float *)mkl_C, 2*size_mkl_C, (float *)alpha_C,
      // 2*size_alpha_C);
      alpha_free(mkl_C);
    }
  }
  ...
  // { // block col major
  //     alpha_spmmd(argc, argv, file, thread_num, &alpha_C, &size_alpha_C,
  //     ALPHA_SPARSE_LAYOUT_COLUMN_MAJOR);
