...
static void mkl_mv_coo(const int argc, const char *argv[], const char *file, int thread_num,
                       const float alpha, const float beta, float **ret_y, size_t *ret_size_y) {
  MKL_INT m, k, nnz;
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    mkl_call_exit(mkl_sparse_s_mv(transA, alpha, cooA, descr, x, beta, y), "mkl_sparse_s_mv");
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    // printf("mkl20 vs ict\n");
    // status = check_s((float *)mkl_y20, size_mkl_y20, (float *)alpha_y,
    // size_alpha_y);

    // //sanity check
    // printf("mkl vs mkl_coo\n");
    // status = check_s((float *)mkl_y, size_mkl_y, (float *)mkl_ycoo,
    // size_mkl_ycoo); printf("mkl20 vs mkl_coo\n"); status = check_s((float
    // *)mkl_y20, size_mkl_y20, (float *)mkl_ycoo, size_mkl_ycoo);
    // // printf("mkl sanity check\n");
    // printf("mkl vs mkl20\n");
    // status = check_s((float *)mkl_y, size_mkl_y, (float *)mkl_y20,
    // size_mkl_y20);
    ...
    // status = check_s((float *)alpha_y, size_alpha_y, (float *)mkl_ycoo,
    // size_mkl_ycoo);
