...
static void alpha_trsm(const int argc, const char *argv[], const char *file,
                     ...
                     ALPHA_Complex8 **ret, size_t *size, ALPHA_INT *ret_ldc) {
  ALPHA_INT m, k, nnz;
  ...
  alpha_timer_t timer;
  alpha_timing_start(&timer);
  alpha_call_exit(alphasparse_c_trsm_plain(transA, alpha, csrA, descr, layout, x,
                                        columns, ldx, y, ldy),
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    //        size_alpha_y * 2);
    status = check_c_l3((ALPHA_Complex8 *)mkl_y, ldc, size_mkl_y, alpha_y, ldc,
