...
static void mkl_spmm(const int argc, const char *argv[], const char *file,
  ...
  // mkl_call_exit(mkl_sparse_d_create_coo(&coo, SPARSE_INDEX_BASE_ZERO, m, k,
  // nnz, row_index, col_index, values), "mkl_sparse_d_create_coo"); if(transA
  ...
  // mkl_call_exit(mkl_sparse_d_create_coo(&coo, SPARSE_INDEX_BASE_ZERO, m, k,
  // nnz, row_index, col_index, values), "mkl_sparse_d_create_coo");
  ...
  mkl_call_exit(mkl_sparse_d_export_csc(result, ret_index, ret_rows, ret_cols,
...
// **ret_col_index, double **ret_values)
static void alpha_spmm(const int argc, const char *argv[], const char *file,
  ...
  alpha_call_exit(alphasparse_d_export_csc(result, ret_index, ret_rows, ret_cols,
                                        ret_rows_start, ret_rows_end,
                                        ret_col_index, ret_values),
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    int alpha_nnz = alpha_rows_end[alpha_cols - 1];
    // status = check_d(mkl_values, mkl_nnz, alpha_values, alpha_nnz);
    double zero = 0.f;
    status =
        check_d_l3((double *)mkl_values, 0, mkl_nnz, alpha_values, 0, alpha_nnz,
