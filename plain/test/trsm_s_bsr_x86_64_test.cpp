...
static void mkl_trsm(const int argc, const char *argv[], const char *file,
  ...
  mkl_call_exit(mkl_sparse_convert_bsr(cooA, BLOCK_SIZE, layout,
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    mkl_call_exit(mkl_sparse_s_trsm(transA, alpha, bsrA, descr, layout, x,
                                    columns, ldx, y, ldy),
                  "mkl_sparse_s_trsm");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter, "mkl_sparse_s_trsm",
         total_time / iter);
  // alpha_timing_end(&timer);
...
static void mkl_trsm_bsr(const int argc, const char *argv[], const char *file,
                         int thread_num, const float alpha, float **ret,
                         size_t *size) {
  MKL_INT m, k, nnz;
  MKL_INT *row_index, *col_index;
  ...
  // #ifdef DEBUG
  //     printf(" mkl_sparse_s_create_coo starts \n");
  // #endif
  mkl_sparse_s_create_coo(&cooA, base, m, k, nnz, row_index, col_index, values);
  // #ifdef DEBUG
  //     printf(" mkl_sparse_s_create_coo ends \n");
  // #endif
  // #ifdef DEBUG
  //     printf(" mkl_sparse_convert_bsr starts \n");
  // #endif
  mkl_sparse_convert_bsr(cooA, block_size, b_layout,
                         SPARSE_OPERATION_NON_TRANSPOSE, &bsrA);
  // #ifdef DEBUG
  ...
  // alpha_timing_start(&timer);
  // #ifdef DEBUG
  //    //     printf(" mkl_sparse_s_trsm starts \n");
  // #endif
...
static void alpha_trsm(const int argc, const char *argv[], const char *file,
  ...
  alpha_call_exit(
      alphasparse_convert_bsr(cooA, block_size, b_layout,
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    alpha_call_exit(alphasparse_s_trsm_plain(transA, alpha, bsrA, descr, b_layout,
                                          x, columns, ldx, y, ldy),
                  "alphasparse_s_trsm");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  ...
  printf("iter is %d, %s avg time : %lf[sec]\n", iter,
         "alphasparse_s_trsm_plain", total_time / iter);
  ;
  // alpha_timing_end(&timer);
