...
static void mkl_trsv(const int argc, const char *argv[], const char *file,
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    mkl_call_exit(mkl_sparse_s_trsv(transA, alpha, bsrA, descr, x, y),
                  "mkl_sparse_s_trsv");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter, "mkl_sparse_s_trsv",
         total_time / iter);
  // alpha_timing_end(&timer);
...
static void alpha_trsv(const int argc, const char *argv[], const char *file,
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    alpha_call_exit(alphasparse_s_trsv_plain(transA, alpha, bsrA, descr, x, y),
                  "alphasparse_s_trsv");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter,
         "alphasparse_s_trsv_plain", total_time / iter);
  ;
...
int main(int argc, const char *argv[]) {
  ...
  //     *)mkl_y_col, size_mkl_y_col, (float *)alpha_y_col, size_alpha_y_col);
