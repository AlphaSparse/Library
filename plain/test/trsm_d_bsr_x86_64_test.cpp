...
static void mkl_trsm(const int argc, const char *argv[], const char *file, int thread_num,
                     const double alpha, double **ret, size_t *size) {
  MKL_INT m, k, nnz;
  ...
  mkl_call_exit(
      mkl_sparse_convert_bsr(cooA, BLOCK_SIZE, layout, SPARSE_OPERATION_NON_TRANSPOSE, &bsrA),
  ...
  for (int i = 0; i < iter; i++) {
    alpha_clear_cache();
    alpha_timing_start(&timer);
    mkl_call_exit(mkl_sparse_d_trsm(transA, alpha, bsrA, descr, layout, x, columns, ldx, y, ldy),
                  "mkl_sparse_d_trsm");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter, "mkl_sparse_d_trsm", total_time / iter);
  // alpha_timing_end(&timer);
...
static void mkl_trsm_bsr(const int argc, const char *argv[], const char *file, int thread_num,
                         const double alpha, double **ret, size_t *size) {
  MKL_INT m, k, nnz;
  MKL_INT *row_index, *col_index;
  double *values;
  ...
  // #ifdef DEBUG
  //     printf(" mkl_sparse_d_create_coo starts \n");
  // #endif
  mkl_sparse_d_create_coo(&cooA, base, m, k, nnz, row_index, col_index, values);
  // #ifdef DEBUG
  //     printf(" mkl_sparse_d_create_coo ends \n");
  // #endif
  // #ifdef DEBUG
  ...
  // alpha_timing_start(&timer);
  // #ifdef DEBUG
  //    //     printf(" mkl_sparse_d_trsm starts \n");
  // #endif
...
static void alpha_trsm(const int argc, const char *argv[], const char *file, int thread_num,
                     const double alpha, double **ret, size_t *size) {
  ALPHA_INT m, k, nnz;
  ...
  alpha_call_exit(
      alphasparse_convert_bsr(cooA, block_size, b_layout, ALPHA_SPARSE_OPERATION_NON_TRANSPOSE, &bsrA),
  ...
  for (int i = 0; i < iter; i++) {
    ...
    alpha_timing_start(&timer);
    alpha_call_exit(
        alphasparse_d_trsm_plain(transA, alpha, bsrA, descr, b_layout, x, columns, ldx, y, ldy),
        "alphasparse_d_trsm");
    alpha_timing_end(&timer);
    total_time += alpha_timing_elapsed_time(&timer);
  }
  printf("iter is %d, %s avg time : %lf[sec]\n", iter, "alphasparse_d_trsm_plain",
         total_time / iter);
  ;
  // alpha_timing_end(&timer);
...
int main(int argc, const char *argv[]) {
  ...
  if (check) {
    ...
    //         // else
    //         // mkl_trsm(argc, argv, file, thread_num, mkl_alpha, &mkl_y,
    //         &size_mkl_y);

    // // #ifdef DEBUG
    // //     printf("size_alpha_y is %ld\n",size_alpha_y);
    // //     for(size_t i = 0 ; i < size_alpha_y;i++)
    // //         printf("%f, %f dist %f \n", alpha_y[i].real , mkl_y[i].real,
    // alpha_y[i].real - mkl_y[i].real);
    // // #endif
